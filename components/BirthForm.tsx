...
export default function BirthForm({ onSubmit, loading, initialData, onFormSave, hideSubmit }: BirthFormProps) {
  ...
  // ─── 样式变量 ────────────────────────────────────────────
  const bg = 'var(--birth-form-bg)';
  const border = 'var(--birth-form-border)';
  const labelClr = 'var(--birth-label)';
  const inputBg = 'var(--birth-input-bg)';
  const inputBorder = 'var(--birth-input-border)';
  const inputClr = 'var(--birth-input-text)';
  const focusBorder = 'var(--birth-input-focus)';
  const errorClr = 'var(--birth-error)';
  const panelBg = 'var(--birth-panel-bg)';
  const panelBorder = 'var(--birth-panel-border)';
  const summaryBg = 'var(--birth-summary-bg)';
  const summaryBorder = 'var(--birth-summary-border)';
  const summaryClr = 'var(--birth-summary-text)';
  ...
  return (
    <motion.form
      ...
      style={{ background: bg, border: `1px solid ${border}`, borderRadius: '8px', padding: '28px', boxShadow: 'var(--sh-sm)' }}
    >
      {/* 标题 */}
      <h3 className="birth-form-title">
        ── 输入生辰八字 ──
      </h3>
      ...
      <div style={{ display: 'flex', gap: '4px', marginBottom: '20px' }}>
        {steps.map((done, i) => (
          <motion.div
            key={i}
            animate={{ background: done ? 'var(--birth-progress-done)' : 'var(--birth-progress-rest)' }}
        ...
        <AnimatePresence mode="wait">
          {form.province ? (
            <motion.p
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              style={{ fontSize: '10px', color: 'var(--birth-helper-text)', marginTop: '5px' }}
            >
            ...
            <motion.p
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              style={{ fontSize: '10px', color: 'var(--birth-muted-text)', marginTop: '5px' }}
            >
      ...
      <div style={{ marginBottom: '16px' }}>
        ...
        <div style={{ borderRadius: '8px', padding: '12px', background: panelBg, border: `1px solid ${panelBorder}`, opacity: form.unknownTime ? 0.45 : 1, pointerEvents: form.unknownTime ? 'none' : 'auto', transition: 'opacity 0.2s' }}>
          ...
          <div style={{ textAlign: 'center', padding: '4px 0' }}>
            <span style={{ fontSize: '10px', color: 'var(--birth-helper-text)' }}>真太阳时 → </span>
            <span className="birth-form-solar-branch">
              {SHICHEN_NAMES[branch]}时
            </span>
            {shichenInfo && (
              <span style={{ fontSize: '10px', color: 'var(--birth-helper-text)', marginLeft: '4px' }}>
                （{shichenInfo.range}）
              </span>
            )}
          </div>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: '7px', marginTop: '8px', cursor: 'pointer' }}>
          ...
          />
          <span style={{ fontSize: '10px', color: 'var(--birth-muted-text)' }}>
            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  color: active ? `rgba(${accent},0.9)` : 'var(--birth-gender-inactive)',
                  transition: 'all 0.2s',
                  cursor: 'pointer',
                }}
              >
      ...
      {!hideSubmit && <motion.button
        ...
        style={{
          ...
          background: loading ? 'var(--ac-bg)' : 'var(--ac)',
          color: loading ? 'var(--birth-submit-disabled-text)' : 'var(--birth-submit-text)',
          boxShadow: loading ? 'none' : 'var(--birth-submit-shadow)',
          transition: 'all 0.2s',
        }}
      >
        {loading ? (
